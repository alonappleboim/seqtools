...
class Sample(object):
    ...
    def make_tracks(files):
        ...
        def handle_strand(char, fout, negate=False):
            ...
            sbed.wait()
            bw = sp.Popen([EXEC['BG2W'], files['tmp_bed'], COMMON_GENOMES['SCER']['chrlens'], fout])
            bw.wait()
...
class ExperimentHandler(object):
    ...
    def parse_sample_db(self):
        ...
        if exp is None:
            ...
            self.log(lg.CRITICAL, msg)
            raise ValueError(msg)
        meta = (getpass.getuser(), proj.group(1), exp.group(1))
        msg = 'user: %s, project: %s, experiment: %s' % meta
        self.log(lg.INFO, msg)
        ...
        return meta, bc_len, b2s, features
    ...
    def setup_output(self):
        if self.a.output_dir is None:
            folder = canonic_path(RNA_DATA_PATH) + os.sep + self.user
            self.dir_and_log(folder)
            folder += os.sep + self.proj
            self.dir_and_log(folder)
        ...
        if self.a.no_hub:
            if self.a.hub_name is None: self.a.hub_name = '-'.join([self.proj,self.exp])
            self.www_rel = os.sep.join([self.proj, self.exp])
            d = os.sep.join([WWW_PATH, self.proj])
            self.dir_and_log(d, lg.INFO, chto='777')
            d = os.sep.join([d, self.exp])
            if os.path.isdir(d):
    ...
    def build_hub(self):
        ...
        genomesfile.write("genome %s\n"
                          "trackDb trackDB.txt" % COMMON_GENOMES['SCER']['assembly'])
        trackfile = open(self.www_path + os.path.sep + 'trackDB.txt', 'w')
        for s in self.samples.values():
...
def build_parser():
    ...
    g.add_argument('--align_index_path', '-aip', type=str, default=COMMON_GENOMES['SCER']['bowtie_index'],
