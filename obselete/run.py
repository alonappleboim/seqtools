...
class MainHandler(object):
    ...
    def __init__(self, argobj, cmdline):
        ...
        for ex, err in self.check_third_party():
            if err is not None:
                self.logger.log(lg.CRITICAL, 'Fatal error with executable %s: %s' % (ex, err))
                exit()

        self.bc_len, self.samples, self.features = self.parse_sample_db()
        self.dirs = self.generate_dir_tree()
    ...
    def count(self):
        ...
        # prepare input bed file for all work
    ...
    def generate_dir_tree(user, exp, debug, out_dir=None):
        out, logs = {}, []
        if out_dir is None:
            folder = canonic_path(DATA_PATH) + os.sep + user
            logs.append(create_dir(folder))
            folder += os.sep + exp
            logs.append(create_dir(folder))
            ...
            out_dir = folder
        d = out_dir
        tmp_dir = d + os.sep + TMP_NAME
        fastq_dir = d + os.sep + fastq_dirname
        bw_dir = d + os.sep + bigwig_dirname
        bam_dir = d + os.sep + self.bam_dirname
        ...
        if self.start_after == BEGIN:
            ...
            self.create_dir(d, lg.INFO)
            self.create_dir(self.fastq_dir)
            self.create_dir(self.bam_dir)
            self.create_dir(self.bw_dir)
            self.create_dir(self.tmp_dir)
            if self.keep_filtered:
                self.filtered_dir = d + os.sep + FILTERED_NAME
                self.create_dir(self.filtered_dir)
            if self.keep_unaligned:
                self.unaligned_dir = d + os.sep + UNALIGNED_NAME
                self.create_dir(self.unaligned_dir)
        ...
        if self.make_hub:
            ...
            self.create_dir(d, lg.INFO,chto='777')
    ...
    def create_dir(self, path, level=lg.DEBUG, chto='770'):
